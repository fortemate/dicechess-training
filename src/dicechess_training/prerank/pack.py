...
def pack(
    ...
    *,
    ...
    # Admitted before it is written, not after. `groups.admit` runs the same checks `load_groups`
    # runs, against the files in this directory, on a manifest that is still only in memory — so
    # a refused corpus is left exactly as it was found. Writing first and undoing on refusal
    # would give the same outcome only for as long as the undo kept working, and would destroy a
    # previously valid manifest every time it had anything to do.
    admitted, groups = admit(manifest, directory)
