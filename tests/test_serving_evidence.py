...
def test_destinations_sharing_a_basename_both_publish(tmp_path, candidate):
    """Two directories, one filename. Staging used to collapse them onto the same staged file."""
    raw_directory, evidence_directory = tmp_path / "raw", tmp_path / "evidence"
    raw_directory.mkdir()
    evidence_directory.mkdir()
    observations = _staged(tmp_path, _observations(candidate), "same-name")
    result = build_evidence(
        candidate,
        FIXTURE,
        [FIXTURE],
        observations,
        evidence_directory / "document.json",
        raw_directory / "document.json",
    )
    published_raw = json.loads((raw_directory / "document.json").read_text(encoding="utf-8"))
    published_evidence = json.loads(
        (evidence_directory / "document.json").read_text(encoding="utf-8")
    )
    assert published_raw["schema"] == "playground-serving-raw-v1"
    assert published_evidence["schema"] == EVIDENCE_SCHEMA
    # And the digest still describes the raw document that was actually published.
    assert published_evidence["raw_evidence_sha256"] == kcp13.sha256_of(
        raw_directory / "document.json"
    )
    assert result["evidence"] == published_evidence


def test_writing_both_documents_to_one_location_is_refused(tmp_path, candidate):
    observations = _staged(tmp_path, _observations(candidate), "one-place")
    destination = tmp_path / "both.json"
